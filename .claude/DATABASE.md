# KruiseKonnect — Database Schema, Indexes & RLS Policies

---

## Rules for All Database Work

1. Every table has RLS enabled — `ALTER TABLE x ENABLE ROW LEVEL SECURITY`
2. The Supabase service role key bypasses RLS — use it ONLY in NestJS backend, never frontend
3. Never use sequential integer IDs as public-facing identifiers
4. `booking_reference` is a random alphanumeric string (format: `KK-XXXXXXXX`)
5. All timestamps are UTC, stored as `TIMESTAMPTZ`
6. Soft-delete only — never hard-delete bookings, payments, or users
7. `audit_logs` and `email_logs` are append-only — no UPDATE or DELETE ever
...
-- ============================================================
-- USERS (managed by Supabase Auth — do not create auth.users)
-- ============================================================
...
CREATE TABLE public.bookings (
  ...
  booking_reference TEXT UNIQUE NOT NULL,  -- format: KK-XXXXXXXX (random, server-generated)
  status            TEXT NOT NULL DEFAULT 'pending'
                    CHECK (status IN ('pending', 'confirmed', 'cancelled', 'failed', 'refunded')),
  total_price       NUMERIC(12, 2) NOT NULL,  -- set at booking creation, immutable after
...
CREATE TABLE public.payments (
  ...
  amount              NUMERIC(12, 2) NOT NULL,  -- in NGN kobo (Paystack unit)
  currency            TEXT NOT NULL DEFAULT 'NGN',
  status              TEXT NOT NULL DEFAULT 'initialized'
                      CHECK (status IN ('initialized', 'pending', 'success', 'failed', 'reversed')),
  paystack_response   JSONB,  -- store full Paystack response for audit
  ip_address          INET,
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
## Indexes (All Required — Performance Critical)
...
-- ============================================================
-- AIRCRAFT & ROUTES & SCHEDULES (public read, admin write)
-- ============================================================
...
-- Aircraft/Routes/Schedules write: service role only (NestJS backend)
-- No client-side INSERT/UPDATE/DELETE allowed on these tables
...
-- No client-side INSERT/UPDATE on bookings — service role only
-- All booking mutations go through NestJS
...
-- ============================================================
-- PAYMENTS — Users can view, never write
-- ============================================================
...
-- No INSERT/UPDATE on payments from client — service role only
...
-- ============================================================
-- EMAIL LOGS — No client access
-- ============================================================
-- No policies for anon or authenticated users
-- Service role only

-- ============================================================
-- ADMIN USERS — No client access
-- ============================================================
-- No policies for anon or authenticated users
-- Service role only

-- ============================================================
-- AUDIT LOGS — No client access, append-only
-- ============================================================
-- No policies for anon or authenticated users
-- Service role only, INSERT only (never UPDATE or DELETE)
```

---

## Atomic Seat Booking (Race Condition Prevention)

This SQL MUST be used inside a transaction when creating a booking.
Never check and decrement in separate queries.

```sql
-- Step 1: Atomically decrement seats — returns nothing if not enough seats
...
-- If RETURNING returns no rows → reject booking with "No seats available"
-- If RETURNING returns a row → proceed to create booking

-- Step 2: Create booking in same transaction
INSERT INTO public.bookings (
  user_id, schedule_id, booking_reference, status, total_price, passenger_count
) VALUES (
  $1, $2, $3, 'pending', $4, $5
) RETURNING *;
```

Both steps MUST be in a single `BEGIN ... COMMIT` transaction.
If the booking insert fails, seats are rolled back automatically.
...
```typescript
// Generate in NestJS, never on client
